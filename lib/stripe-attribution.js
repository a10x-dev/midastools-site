...
function getLandingSlug() {
  ...
  // Root path strips to '' which is falsy and gets dropped from attribution,
  // making homepage-direct landings indistinguishable from missing attribution.
  // Record the homepage explicitly as 'home' so the acquisition funnel can tell
  // "landed on brand homepage" apart from "no attribution captured".
  return window.location.pathname.replace(/^\/+/, '').slice(0, 60) || 'home';
}
