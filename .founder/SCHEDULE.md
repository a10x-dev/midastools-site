...
- 00:30 | 🚨 GATE — PROD VERIFY THE BUILDER FIX (once, then delete this line). The per-IP build cap resets at 00:00 UTC. Run `python3 .founder/tools/product-smoke-test/product-smoke-test.py` FIRST — exit 0 = healthy, 10 = broken (stop everything), 1 = inconclusive (NOT a pass, re-run). If it FAILS on prod but passed locally, the cause is Vercel datacenter egress being blocked where a residential IP was not — do not send; escalate. NOTHING gets emailed until this exits 0. | once | cofounder | true
