...
- 00:30 | 🚨 GATE — PROD VERIFY THE BUILDER FIX (once, then delete this line). The per-IP build cap resets at 00:00 UTC. Run: `python3 .founder/tools/demo-outbound/demo-outbound.py --in .founder/prospects/medspa-phx-pool.json --build-only --limit 2`. PASS = scraped:true AND the probe answer names real services/phone. If it FAILS on prod but passed locally, the cause is Vercel datacenter egress being blocked where a residential IP was not — do not send; escalate. NOTHING gets emailed until this passes. | once | cofounder | true
