...
# ✅ DONE Jun 15 19:50 UTC (S40): memo_art_money FIRED → sent:153, failed:0. sends=153 = kill-criterion denominator. Pre-flight clean (ready:true, 3×200, 0 baseline). Send entry below DISABLED.
- 09:00 | DONE — memo_art_money sent:153 on Jun 15 (S40). | once-june15 | cofounder | false
- 09:00 | 🔴 KILL-READ (the whole point of the Jun-15 send) — run ~24-48h after the Jun-15 send, i.e. Jun 16-17. ONE command: `python3 .founder/tools/funnel-readout.py --campaign memo_art_money --sends 153` → prints `METHOD-CTR = pv/153 = X.XX% -> KILL/PASS (threshold 2.0%)`. ⚠️ Do NOT read the "% of signups" line — WRONG denominator for a broadcast (overstates ~5x, S37). KILL the memo lever if <2% method-CTR (this is issue #1 of the 4-issue criterion). Also watch any resulting sell_path_click + any reply to the memo's reply-prompt (demand-sensing for the next money-method). Opens trickle over 24-48h (S33) so reading before ~12-18h is uninformative. | once-june16 | cofounder | true
# 🔥 MEMO #2 — Coloring Book Machine launch ($9.99/book, gift-first + sell-on-KDP). Approved by Armando in pair session Jun 16. Send entry DONE.
- 09:00 | ✅ DONE Jun 22 20:15 UTC (S25): coloring_book_launch FIRED → sent:203, failed:0. Pre-flight clean (page 200, GEMINI real image, 0 baseline, preview success). sends=203 = kill-criterion denominator. | once-june22 | cofounder | false
- 09:00 | 🔴 KILL-READ #2 (the whole point of the Jun-22 send) — run ~24-48h after, i.e. Jun 23-24 (after ~12-18h; opens trickle so an earlier read is uninformative). ONE command: `python3 .founder/tools/funnel-readout.py --campaign coloring_book_launch --sends 203` → prints `METHOD-CTR = pv/203 = X.XX% -> KILL/PASS (threshold 2.0%)`. ⚠️ Do NOT read the "% of signups" line — WRONG denominator for a broadcast (overstates ~5x, S37). Also watch Stripe for the FIRST $9.99 coloring-book sale (webhook subject "🎨 SALE: Coloring Book Machine") — this is the first real revenue test of the product + any reply to the memo's reply-prompt (demand-sensing for next Monday's build). | once-june23 | cofounder | true
