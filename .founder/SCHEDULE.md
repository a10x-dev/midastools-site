...
# GATE CLEARED 2026-08-06 00:05 UTC — smoke test PASSED on prod (cb_90d1e9fdc3cc, scraped, grounded, refused the trap). 18/18 demos then sent. Line retired.
- 08:00 | Daily: PRODUCT SMOKE TEST — `python3 .founder/tools/product-smoke-test/product-smoke-test.py` — asserts the Chatbot Builder produces a WORKING bot (scraped, grounded, refuses to invent), not just an HTTP 200. Exit 10 = product broken, halt outbound and page Armando. This is the instrument whose absence let the flagship die silently for weeks. Costs 1 of the 8 daily builds — run it BEFORE the outbound batch. | daily | cofounder | true
- 10:00 | OUTBOUND BATCH — run the next batch of `demo-outbound.py`. MEASURED 2026-08-06: the build cap is ~50/day, NOT 8 (the response reports `remaining=` — read it, don't assume). Source prospects first; never send to a bot whose build returned scraped:false. HOLD new batches until the 18-prospect Phoenix cohort has been read at 24h and 72h — do not scale a channel before its first cohort reports. | daily | cofounder | true
- 16:00 | OUTBOUND READ — `python3 .founder/tools/outbound-read/outbound-read.py` — per-PROSPECT demo opens. Exit 10 = WARM LEAD, follow up within 24h with a 2-line reply offering to put it on their site. Exit 1 = measurement broken, do NOT read as "nobody opened". This is the metric that decides whether outbound lives or dies. Baseline: 0/18 at 2026-08-06 00:20 UTC. | daily | cofounder | true
