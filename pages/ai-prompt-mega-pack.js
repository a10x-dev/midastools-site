...
export default function AIPromptMegaPack() {
  ...
  const handleLead = async (e) => {
    ...
    setLoading(true);
    try {
      await submitSubscribe({ email, source: 'prompt-mega-pack-lead', business: 'General' });
      setSent(true);
    } catch {}
    setLoading(false);
  };
...
- Include 1 numbered list and 1 bulleted list minimum`}</div>
      ...
      <section style={{paddingTop:0}}>
        <div style={{background:'rgba(59,95,255,0.04)', border:'1px solid rgba(59,95,255,0.15)', borderRadius:16, padding:32, textAlign:'center'}}>
          ...
          <p style={{fontSize:20, fontWeight:800, marginBottom:4, marginTop:8}}>Get all 16 AI kits for $97</p>
